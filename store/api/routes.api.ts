...
> = async (args, api, extraOptions) => {
  ...
  if (result.error && result.error.status === 401) {
    ...
    const refreshResult: any = await refreshQuery(`/refresh`, api, extraOptions);
    // store the new token
    ...
    if (!refreshResult.error) {      
      ...
      await EncryptedStorage.setItem('refresh', refreshResult.data.refreshToken);
...
export const { useSaveRouteMutation, useGetAllRoutesQuery, useDeleteRouteMutation, useUpdateRouteMutation, useApproveRouteMutation } =
  routesApi;
